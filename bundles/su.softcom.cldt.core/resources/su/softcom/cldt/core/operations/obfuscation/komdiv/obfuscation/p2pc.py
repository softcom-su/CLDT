...
for lu in F:
	...
	if not fn or lu["func"] in FN:
		c, c3, c4 = deadcode(lu, PC)			# внедрение мертвого кода
		if c3: 
...
print(sys.argv[0], " - внедрение мертвого кода с вероятностью".encode("utf-8"), sys.argv[3] + "%", "в каждой точке внедрения".encode("utf-8"))
...
print("       точек внедрения   :".encode("utf-8"), C4)
print("Добавлено новых команд   :".encode("utf-8"), C3)
print("Количество участков с мертвым кодом :".encode("utf-8"), L1)
FC.sort()
LU.sort()
print("Измененения внесены".encode("utf-8"))
print("\tв функции :".encode("utf-8"), FC)
print("\tв участки :".encode("utf-8"), LU)
