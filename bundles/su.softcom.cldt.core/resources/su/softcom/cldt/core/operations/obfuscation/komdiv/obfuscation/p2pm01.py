...
#print(sys.argv[0], " - перемешение и кодирование участков с вероятностью".encode("utf-8"), sys.argv[3] + "% для функций :".encode("utf-8"), " ".join(sys.argv[4:]))
#print("\nИсходный файл : ".encode("utf-8") + sys.argv[1])

K, MV, MC = mselect(F, FN, fn, PC)		# Кандидатов (K), выбраны для перемещения (MV), кодирования с перемещением (MC)
...
for i, lu in enumerate(F):
	if lu["func"] in FN:
		if "free" not in P or lu["func"] not in P["free"] or len(P["free"][lu["func"]]) < 5:
			...
			if lu["lunum"] in MV: F[i] = set_mv(lu, MCL)
			if lu["lunum"] in MC: F[i] = set_mc(lu, MCL)
#if not fn:
#	print("     всего участков    : ".encode("utf-8") + str(len(F)))
#else:
#	print("     всего участков    : ".encode("utf-8") + str(flen(F, FN)))
#print("     из них кандидатов : ".encode("utf-8") + str(len(K)))
pf(F, K)
#print("\nВыходной файл : ".encode("utf-8") + sys.argv[2])
#print("     выбрано участков для перемещения : ".encode("utf-8") + str(len(MV)))
pf(F, MV)
#print("     для кодирования с перемещением   : ".encode("utf-8") + str(len(MC)))
pf(F, MC)
