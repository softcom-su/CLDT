...
fwc(" Файл сгенерирован автоматически", f2)
fwc("\t" + "  ".join(sys.argv), f2)
if len(GD):
	fwc(" === Глобальные директивы", f2)
	fwt(".text", f2)
	for line in GD:	fwt(line, f2)
if len(G):
	fwc(" === Глобальные символы", f2)
	for line in G:	fwt(".globl\t" + line, f2)
if len(L):
	fwc(" === Локальные символы", f2)
	for line in L:	fwt(".local\t" + line, f2)
if len(GS):
	fwc(" === Глобальные установки", f2)
	for line in GS:	fwt(".set\t"   + line, f2)
if len(FS):
	fwc(" === Установки секций кода", f2)
	for line in FS: fwt(".set\t"   + line, f2)
fwc(" === Секции кода", f2)
S = ""
for lu in F:
# заголовок - метки и директивы
	if "section" in lu and lu["section"] != S: 
		S = lu["section"]
		fwt(S, f2)
	if "globl"   in lu: fwt(".globl\t" + lu["globl"], f2)
	if "align"   in lu: fwt(".align\t" + lu["align"], f2)
	if "p2align" in lu: fwt(".p2align\t" + lu["p2align"], f2)
	if "label"   in lu: fw (lu["label"] + ":", f2)
	fw("_LU_" + str(lu["lunum"]) + ":", f2)
# перемещение и кодирование
	if "move" in lu: mv_code(lu, P["free"][lu["func"]], f2)
# тело участка
	for l in lu["txt"]: 
		if len(l): fc(l, f2)
# команда перехода и слот
	if "cmd"  in lu: fc(lu["cmd"], f2)
	if "slot" in lu: fc(lu["slot"], f2)
# команда перехода на следующий участок при перемешивании
	if shuf and "next" in lu:
		fc(["ShuffleJmp", "j", "_LU_" + str(lu["next"])], f2)
		fc(["ShuffleJmp", "nop"], f2)
if len(DS):
	fwc(" === Установки секций данных", f2)
	for line in DS:	fwt(".set\t"   + line, f2)
if len(D):
	fwc(" === Секции данных", f2)
	for dat in D:
		for line in dat: fw(line, f2)
if len(DD):
	fwc(" === Секции мертвых данных", f2)
	for dat in DD:
		for line in dat: fw(line, f2)
if len(MC):
	fwc(" === Память для перемещаемых участков", f2)
	for line in MC: fw(line, f2)
f2.close()
