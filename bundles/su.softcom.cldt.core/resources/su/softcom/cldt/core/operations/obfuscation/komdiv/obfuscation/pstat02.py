#!/usr/bin/env python3
#
# Сравнительная статистика по количеству участков и внешних дуг для файлов .p.json одной и той же ПП, (С), Als, 06/2021
#
...
def cmderr():
	print("\t" + sys.argv[0] + " in_file1.p.json  in_file2.p.json")
	sys.exit(2)

if len(sys.argv) != 3: cmderr()

P1  = json.loads(open(sys.argv[1], "r").read())
P2  = json.loads(open(sys.argv[2], "r").read())
A1  = P1["attr"]
F1  = P1["code"]
A2  = P2["attr"]
F2  = P2["code"]

if A1["version"] != "mips.p.0.1":
	print("Неподдерживаемая версия файла " + sys.argv[1]); sys.exit(2)
if A2["version"] != "mips.p.0.1":
	print("Неподдерживаемая версия файла " + sys.argv[2]); sys.exit(2)

L, D, C = stat02(F1, F2)

the_file = open('log.txt', 'w', encoding="utf-8")
log_str = ""
#log_str = log_str + "Функция\t\t\tК-во участков\t\t\tМашинных команд\t\t\tВнешних дуг\n"
...
#	if len(fu) < 20:l += "\t"
	...
	z1 += C[fu][0]
	z2 += C[fu][1]
	log_str = log_str + fu + l + str(L[fu][0]) + "\t" + str(L[fu][1]) + "\t" + str(C[fu][0]) + "\t" + str(C[fu][1]) + "\t" + str(D[fu][0]) + "\t" + str(D[fu][1]) + "\n"
log_str = log_str + "Итого:\t" + str(x1) + "\t" + str(x2) + "\t" + str(z1) + "\t" + str(z2) + "\t" + str(y1) + "\t" + str(y2) + "\n"
