#!/usr/bin/python3
# 
# Общие функции для преобразований текстов МИПС и Комдив64, (С), Als, 04/2021
# 
...
jmp   = ["b", "j", "jr", "jr.hb"]
jcc   = ["bc1f", "bc1fl", "bc1t", "bc1tl", "bc2f", "bc2fl", "bc2t", "bc2tl", "beq", "beql", "bgez", "bgezl", "bgtz", "bgtzl", \
...
exits = ["exit"]
SC    = ['add', 'addi', 'addiu', 'addu', 'and', 'andi', 'cfc1', 'cfc2', 'clo', 'clz', 'dadd', 'daddi', 'daddiu', 'daddu', 'dclo',     \
	 'dclz', 'dmfc0', 'dmfc1', 'dmfc2', 'dneg', 'dnegu', 'drotr', 'drotr32', 'drotrv', 'dsbh', 'dshd', 'dsll', 'dsll32', 'dsllv', \
	 'dsra', 'dsra32', 'dsrav', 'dsrl', 'dsrl32', 'dsrlv', 'dsub', 'dsubu', 'lb', 'lbu', 'ld', 'ldl', 'ldr', 'lh', 'lhu', 'lui',  \
	 'lw', 'lwl', 'lwr', 'lwu', 'mfc0', 'mfc1', 'mfc2', 'mfhc1', 'mfhc2', 'mfhi', 'mflo', 'move', 'movf', 'movn', 'movt', 'movz', \
	 'mul', 'neg', 'negu', 'nor', 'not', 'or', 'ori', 'rotr', 'rotrv', 'sll', 'sllv', 'slt', 'slti', 'sltiu', 'sltu', 'sra',      \
...
def syn(lu):
	...
	for line in lu:
		l1.append([])
		if len(line) > 1 and line[0] in SC and line[1] not in " ".join(line[2:]) and line[1] not in RX: l1[-1].append(line[1])	# не используем регистры из RX
	l2 = copy.deepcopy(l1)
	for i in range(len(l1)-1, -1, -1):
		if len(l1[i]):
			for j in range(i-1, -1, -1):
#				if l1[i][0] not in " ".join(lu[j]):
				if l1[i][0] not in " ".join(lu[j]) and lu[j][0] not in calls:	# [calls] сбрасывает список, ### (!) надо поднять свободный регистр из слота (!) ###
					if l1[i][0] not in l2[j]: l2[j].append(l1[i][0])
				else: break
	for i in range(len(l1)):
		if len(l2[i]): 
			l2[i].sort()
			if (i and lu[i-1][0] not in calls) or not i: l3.append(["free-" + str(len(l2[i]))] + l2[i])	# команда в слоте - без точки внедрения !!!
		l3.append(lu[i])
	return l3
	...
	while i < len(F):
		...
		if len(p):
			if islabel(p[0]):
				...
				if len(C[n]["txt"]) or "label" in C[n]:		# так сработает при двух метках подряд (будет пустой участок с меткой)
					n += 1
					C[n] = {"txt":[]}
				C[n]["label"] = p[0][:-1]
			elif p[0] in jcc:
...
#
def split_lu(lu0, l, n2):		# lu0 - участок, l - рекомендованная длина при дроблении, n2 - номер нового участка при дроблении
	C = []
	c0=0; c1=0; c2=0
	T = lu0["txt"]
	c0 = len(T)
...
import DeadCode as dc

def one_deadcode(r):
	c    = dc.DC[random.randint(0,len(dc.DC)-1)]
	c[1] = r    [random.randint(0,len(r)-1)]
	c[2] = dc.DR[random.randint(0,len(dc.DR)-1)]
	if   c[3] == "R": 
		c[3] = dc.DR[random.randint(0,len(dc.DR)-1)]
	elif c[3] == "N":
		...
		if k:
			if i and i < len(F) - 1:						# не работаем с первым и последним участком
				if len(lu["txt"]) > 1 and len(lu["txt"]) < 11:			# содержит от 2 до 10 машинных команд
					if "next" in F[i-1] and F[i-1]["next"] == lu["lunum"]:	# является следующим по тексту для предыдущего участка
						if targets(F, lu["lunum"]) == 0:		# не является таргетом других участков, включая самого себя (кроме ложных переходов)
							if wrong_cmd(lu["txt"]) == 0:		# не содержит "вредных" команд
#								if wrong_arg(lu["txt"]) == 0:	# не содержит "вредных" аргументов # ### не уверен что %% мешают ###
