// Файл "cos.h" содержит inline версиию функцию математической библиотеки cos().
...
// cos() - функция для вычисления значение косинуса аргумента, заданного в радианах.
...
#if( !defined( __MATH_INLINE_COS_H__ ))
#define __MATH_INLINE_COS_H__

#ifdef BT_MATH_INLINE
#define cos MATH_INLINE_COS
#endif
...
static double __inline__ __UNUSED_F MATH_INLINE_COS( double x )
{
   ...
   if( ABS_X.u64 <= M_PI_4_HEX )
   {
       ...
       {
          // |x| < 2^(-27)
          return( 1.0 );            // cos(x) ~ 1.0
       }
       // |x| <= +pi/4
       return( COS_PI_1_4( x ));    // вычисление cos(x) по формуле Тейлора
   }
   return( __cos_for_large_arg( x ));
}

#endif //__MATH_INLINE_COS_H__
