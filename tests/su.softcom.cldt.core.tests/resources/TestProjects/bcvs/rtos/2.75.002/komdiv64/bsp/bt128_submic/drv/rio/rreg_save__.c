...
int rregDebug =  3;  //gmu:debug 1=>3                       /* debug level */
...
void rregThread(void* hostT)
{
   ...
   RREG_DEBUG(1,(" debug: rregThread start \n",0,0,0,0,0,0)); //gmu:dbg
    ...
    while(1)
    {
        ...
        RREG_DEBUG(1,(" debug: dbVal = 0x%x db = %d id = %d \n",dbVal,db,id,0,0,0)); //gmu:dbg
        RREG_DEBUG(1,(" debug: rregState[%d][%d].txReady = 0x%x \n",db,id,rregState[db][id].txReady,0,0,0)); //gmu:dbg
        RREG_DEBUG(1,(" debug: rregState[%d][%d].rxReady = 0x%x \n",db,id,rregState[db][id].rxReady,0,0,0)); //gmu:dbg
        RREG_DEBUG(1,(" debug: rregState[%d][%d].ready = 0x%x \n\n",db,id,rregState[db][id].ready,0,0,0)); //gmu:dbg
