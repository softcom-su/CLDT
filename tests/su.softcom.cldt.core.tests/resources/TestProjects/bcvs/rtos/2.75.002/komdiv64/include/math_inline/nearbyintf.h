// Файл "nearbyintf.h" содержит inline функцию математической библиотеки float nearbyintf( float x ).
...
// Описание:
//    nearbyintf() - функция для округления аргумента до ближайшего целого числа типа float,
...
#if( !defined( __MATH_INLINE_NEARBYINTF_H__ ))
#define __MATH_INLINE_NEARBYINTF_H__

#ifdef BT_MATH_INLINE
#define nearbyintf MATH_INLINE_NEARBYINTF
#endif

#include <math_inline/fabs.h>

static float __inline__ __UNUSED_F MATH_INLINE_NEARBYINTF( float x )
{
   ...
   if( FLT_INT_IS_ZERO_OR_SUBNORMAL( ABS_X ))
   {
      ...
      if( FLT_INT_IS_MANT_EQ_ZERO( ABS_X ))
      {
         return( x );
      }
...
#endif //__MATH_INLINE_NEARBYINTF_H__
