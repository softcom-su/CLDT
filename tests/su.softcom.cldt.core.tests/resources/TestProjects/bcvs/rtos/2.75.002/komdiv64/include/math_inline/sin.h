// Файл "sin.h" содержит inline версиию функцию математической библиотеки sin().
...
// sin() - функция для вычисления значение синуса аргумента, заданного в радианах.
//
// Синтаксис:
//    #include <math.h>
//    double sin( double x );
...
#if( !defined( __MATH_INLINE_SIN_H__ ))
#define __MATH_INLINE_SIN_H__

#ifdef BT_MATH_INLINE
#define sin MATH_INLINE_SIN
#endif
...
static double __inline__ __UNUSED_F MATH_INLINE_SIN( double x )
{
   DBL_INT_UNION ABS_X = { fabs( x ) }; // двоичное представление модуля аргумента x
   double abs_x, y, z;
   ...
   if( ABS_X.u64 <= M_PI_3_4_HEX )     // безопасное двоичное сравнение чисел (включая NaN)
   {
      ...
      if( abs_x <= M_PI_4 )
      {
         ...
         // |x| <= +pi/4
         return( SIN_PI_1_4( x ));     // формула Тейлора
      }
   ...
   return( __sin_for_large_arg( x ));
}

#endif //__MATH_INLINE_SIN_H__
