// Файл:
//    "scalbf.h" - содержит inline функцию математической библиотеки.
//       float scalbf( float x, float n ).
//
// Описание:
//    scalbf() - функция вычисление числа типа float по мантиссе и порядку
//
// Синтаксис:
//    #include <math.h>
//    float scalbf( float x, float n );
...
//       то будет возвращено плюс или минус HUGE_VALF в зависимости от знака числа и
...
#if( !defined( __MATH_INLINE_SCALBF_H__ ))
#define __MATH_INLINE_SCALBF_H__
...
#define scalbf MATH_INLINE_SCALBF
#endif // BT_MATH_INLINE

static float __inline__ __UNUSED_F MATH_INLINE_SCALBF( float x, float n )
{
   FLT_INT_UNION X = { x }; // целочисленное представления числа типа float со знаком
   FLT_INT_UNION N = { n }; // целочисленное представления числа типа float со знаком
   ...
   {
      // x = [ -Inf, +Inf ]
      if( FLT_INT_EQ( N, FLT_INT_MINUS_INF ))
      {
         // n равен -Inf
         errno = EDOM;
         return( FLT_INT_NAN.flt );
      }
      return( x );
   }
   ...
   if( FLT_INT_IS_ZERO_OR_SUBNORMAL( N ))
   {
   ...
   if( FLT_INT_IS_ZERO_OR_SUBNORMAL( X ))
   {
      ...
      {
         // x = 0.0
         if( FLT_INT_EQ( N, FLT_INT_PLUS_INF ))
         {
            // n равен +Inf
            errno = EDOM;
            return( FLT_INT_NAN.flt );
      ...
      n_sub = fmsbs1( X.u32 ); // ищем старший бит в старшем слове (нумерация с 1)
      n_sub = FLT_INT_MANT_LENGTH - n_sub + 1; // вычисляем величну сдвига
      n -= n_sub; // новое целочисленное значение степени с учетом поправки
      X.u32 <<= (uint32_t)n_sub; // сдвигаем мантиссу влево - умножаем на 2^n
   ...
   if( n < (-2) * FLT_INT_EXP_MAX - 24 )
   {
      // Потеря точности (число меньше чем минимальное неномализованное число)
      return( MATH_INLINE_LDEXPF_FAST( X.flt, INT_MIN ));
   }
   return( MATH_INLINE_LDEXPF_FAST( X.flt, (int)n ));
}

#endif //__MATH_INLINE_SCALBF_H__
