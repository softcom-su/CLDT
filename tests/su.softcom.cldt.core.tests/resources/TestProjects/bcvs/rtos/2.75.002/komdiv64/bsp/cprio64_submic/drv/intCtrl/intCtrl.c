/* intCtrl.c - K64SMP/K64RIO interrupt controlling library */
...
void intrCtrlShow(void)
{
	...
	INTR_REGS->index = INDEX_VECTOR + ix;
	iv = INTR_REGS->data;
        printf(" %02d %s\t: pin=%d  pri=%02d iv=0x%02x %s",ix,
                sk64IntrName[ix],pin,pri,iv,mask ? "OFF" : "ON");

#ifdef	INCLUDE_IV_COUNT
	printf(" count=%d", boardIvCount[iv]);
#endif
        if (iv == IV_IRQ0 || iv == IV_IRQ1)
          { if (iv == IV_IRQ0) mask = *(unsigned char *)IRQ_BASE & 0x35;
            if (iv == IV_IRQ1) mask = *(unsigned char *)IRQ_BASE & 0xca;
            printf ("           ctrl=0x%02x\n\r", mask);
          }
        if (iv == IV_GPIO2 || iv == IV_GPIO3)
          { if (iv == IV_GPIO2) mask = *IO_INT_A & 0x35;
            if (iv == IV_GPIO3) mask = *IO_INT_A & 0xca;
            printf ("           ctrl=0x%02x\n\r", mask);
          }
        printf ("\n\r");
    }
...
#ifdef BT83M       /* making compatibility with bt83b */
/******************************************************************/
int intEnableASIC (int iv_minor)
{ int iv;
  switch (iv_minor)
  { case IV_MINOR_EXT_INT_0: iv = IV_GPIO2; break; 
    case IV_MINOR_EXT_INT_1: iv = IV_GPIO3; break;
    case IV_MINOR_EXT_INT_2: iv = IV_GPIO10; break;
    case IV_MINOR_EXT_INT_3: iv = IV_GPIO11; break;
    default: return (-1);
  }
  intrCtrlMask(iv,TRUE);
  return 0;
}

int intDisableASIC (int iv_minor)
{ int iv;
  switch (iv_minor)
  { case IV_MINOR_EXT_INT_0: iv = IV_GPIO2; break; 
    case IV_MINOR_EXT_INT_1: iv = IV_GPIO3; break;
...
int boardIntDemux(unsigned line)
{
    ...
    // if (iv != IV_UART0) kprint ("inerrupt line %i (%#x) \n\r", iv-IV_BASE, iv-IV_BASE);
    return iv;
}
